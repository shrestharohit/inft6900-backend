...
class Certificate {
    ...
    static async create({ userID, courseID, content, certificateURL }) {
        const query = `
            INSERT INTO "tblCertificate" ("userID", "courseID", "content", "certificateURL", "issueDate")
            VALUES ($1, $2, $3, $4, NOW())
    ...
    static async findByUserId(userID) {
        const query = `SELECT * FROM "tblCertificate" WHERE "userID" = $1 ORDER BY "issueDate" DESC`;
    ...
    static async findByCourseId(courseID) {
        const query = `SELECT * FROM "tblCertificate" WHERE "courseID" = $1 ORDER BY "issueDate" DESC`;
    ...
    static async findById(certificateID) {
        const query = `SELECT * FROM "tblCertificate" WHERE "certificateID" = $1`;
